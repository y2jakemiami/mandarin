...
export function parseAIResponse(responseText) {
    ...
    try {
        ...
        if (jsonStr && jsonStr.startsWith('{')) {
            ...
            if (parsed.feedback) {
                // Проверка: если в feedback есть иероглифы — очищаем
                const hasChinese = /[\u4e00-\u9fff]/.test(parsed.feedback);
                result.feedback = hasChinese ? '' : parsed.feedback;
            }
...
function validateEmotion(emotion) {
    const valid = ['calm', 'happy', 'sad', 'surprised'];
